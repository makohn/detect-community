...
Cluster::Cluster(EgoGraph * graph)
        : graph_(graph), num_(graph->num_nodes_ * graph->num_features_), theta_(
                NULL), alpha_(NULL) {
}
    ...
    for (int rep = 0; rep < reps; rep++) {
        for (int k = 0; k < K; k++) {
            ...
            if (rep == 0 || chat_[k].size() == 0U
                    || static_cast<int>(chat_[k].size()) == M) {
...
double Cluster::loglikelihood(const double * theta, const double * alpha,
    ...
    const int K = static_cast<int>(chat.size());
    const int N = graph_->num_features_;
    const int M = graph_->num_nodes_;
    ...
    for (const auto & i : graph_->edge_features_) {
        ...
        for (int k = 0; k < K; k++) {
            ...
            inp_sum += d * InnerProduct(i.second, theta + k * N, N);
        }
        if (exist) {
            loglike += inp_sum;
        }
...
std::set<int> Cluster::minimize_graphcuts(int k, int improveReps,
        int * changed) {
    const int E = static_cast<int>(graph_->edge_features_.size());
    const int K = static_cast<int>(chat_.size());
    const int N = graph_->num_features_;
    const int M = graph_->num_nodes_;
    QPBO<double> q(M, E);
    q.AddNode(M);
    ...
    for (const auto & i : graph_->edge_features_) {
        ...
        int node0 = e.first, node1 = e.second;
        bool exist = (graph_->edge_.find(e) != graph_->edge_.end());
        double inp_sum = InnerProduct(i.second, theta_ + k * N, N);
        double otp_sum = 0.0;
        for (int l = 0; l < K; l++) {
            if (l == k) continue;
            ...
            otp_sum += d * InnerProduct(i.second, theta_ + l * N, N);
        }
...
void Cluster::dl(double* dldt, double* dlda, int K, double lamda) {
    ...
    for (const auto & i : graph_->edge_features_) {
        ...
        for (int k = 0; k < K; k++) {
            ...
            inp_sum += d
                    * (inps[k] = InnerProduct(i.second, theta_ + k * N, N));
        }

        double expinp = exp(inp_sum), q = expinp / (1 + expinp);
        if (q != q) q = 1;  // in case q is not a number.
...
void Cluster::InitialSet(set<int>* _set, double * theta, double * alpha,
    ...
    std::fill(theta, theta + num_feature, 0.0);
    theta[rand_r(&seed) % num_feature] = 1.0;
    theta[0] = 1.0;
    *alpha = 1.0;
}
