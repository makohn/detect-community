//============================================================================
// Name        : graph_match.cpp
// Author      : ronaflx
// Version     : 1.0
// Copyright   : GNU GPL
// Description : bipartite graph match
//============================================================================
...
int GraphMatch(const vector<set<int> >& one, const vector<set<int> >& that) {
    int n = static_cast<int>(one.size());
    int m = static_cast<int>(that.size());
    int num_node = std::max(n, m);
    Initial(num_node);
    for (int i = 0; i < n; i++) {
        for (int j = 0; j < m; j++) {
